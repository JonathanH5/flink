...
import org.apache.flink.types.{IntValue, Record}
...
object MultinomialNaiveBayes {
    ...
    override def fit(instance: MultinomialNaiveBayes,
      ...
      if (p3 == 0) {
        ...
        pwc = singleWordsInClass.map(new RichMapFunction[(String, String, Int), (String, String, Double)] {
          ...
          var broadcastMap: Map[String, Int] = Map[String, Int]()
      ...
      //TODO don't do that, use broadcasts instead for class info
      //join the pwc data set with the classInfo data set to get one data set as representation of the model and calculate logarithms
  ...
  implicit def predictNNB = new PredictDataSetOperation[MultinomialNaiveBayes, (Int, String), (Int, String)]() {
    ...
    override def predictDataSet(instance: MultinomialNaiveBayes,
      ...
      } //(id -> class name -> word -> wordc ount in text (each word) -> log(P(w|c)) -> log(P(c)) -> log(p(w|c)) not in class)

      //calculate sumpwc for found words
      // 1. Map: only needed information: id -> class -> word count in text (each word) *  log(p(w|c)) each word
      ...
      // 1. Map: only needed information: id -> class -> word count in text (each word) -> log(p(w|c)) not in class
